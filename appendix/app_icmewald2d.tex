...
In this section, we present supplementary results for the numerical validation of error estimates in electrostatic energy, complementing the force results of Chapter~\ref{chp_icmewald2d}. Figures~\ref{fig:icm_error} to~\ref{fig:error_icm_pad_gamma_1} correspond one-to-one with Figs.~\ref{fig:icm_error_force}, \ref{fig:elc_error_force}, \ref{fig:error_icm_pad_gamma_0.6_force} and~\ref{fig:error_icm_pad_gamma_1_force}, maintaining the same systems and parameter settings as those used in the force calculation results. The dashed lines in these figures represent the fitted curves based on our theoretical estimation given in Eq.~\eqref{eq:Uerr}. The energy error results exhibit similar trends to the force errors and align well with our theoretical predictions.
...
\begin{figure}[htbp]
    ...
    \caption{
        Relative errors of energy due to truncation of image charges are presented. The dashed lines represent the fitted decay rate, as described in Eq.~\eqref{eq:Uerr}. In panel (a) we set $\gamma_{\T u} = \gamma_{\T d} = 1$ and consider system heights of $H = 0.5, 1$ and $5$. In panel (b), we set $H = 1$ while varying $\gamma_{\T u} = \gamma_{\T d} = \gamma = 0.3, 0.6$ and $1$. 
...
\begin{figure}[htbp]
    ...
    \caption{Relative error of energy of reformulating the Ewald2D summation as 3D ones is presented. We set $\gamma_{\T u} = \gamma_{\T d} = 0$ and consider system heights of $H = 0.5, 1, 5$. Here $R = (L_z - H) / L_x$ denotes the padding ratio.}
    \label{fig:elc_error}
\end{figure}
...
To further investigate the numerical properties of strongly-confined systems, we present the errors in force in Fig.~\ref{fig:icm_elc_error_force_app}, where we fix $L_x=L_y=10$, $R=(L_z-H)/L_x = 5$ and consider system heights $H = 0.5, 1, 5$ while varying the number of image charge layers $M$. 
In Fig.~\ref{fig:icm_elc_error_force_app} (a), for $\gamma = 0.6$, we observe that the error decays exponentially as $M$ increases for $H = 0.5$ and $H = 1$. 
However, for $H = 5$, where $|g_{\T u}g_{\T d}|>1$, the error becomes non-monotonic as $M$ increases, 
which is consistent with the theoretical predictions of Chapter~\ref{chp_icmewald2d}.
In Fig.~\ref{fig:icm_elc_error_force_app} (b), with $\gamma = 1$, we observe a similar non-monotonic pattern for all aspect ratios. 
...
The same conclusions hold for the errors in energy, as shown in Fig.~\ref{fig:icm_elc_error}.
...
\begin{figure}[htbp]
    ...
    \label{fig:icm_elc_error_force_app}
\end{figure}
