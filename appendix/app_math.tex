...
In this appendix, we state several fundamental results from Fourier analysis for doubly periodic functions, associated with the Fourier transform pair defined in Definition~\ref{Def::Fourier}. These results are useful to us, and their proofs are well established and can be referenced in classical literature, such as in the work of Stein and Shakarchi~\cite{stein2011fourier}.
\begin{lem}\label{lem::Convolution}
	(Convolution theorem) Let $f(\bm{\rho},z)$ and $g(\bm{\rho},z)$ be two functions that are periodic in $\bm{\rho}$ and non-periodic in $z$. Suppose that $f$ and $g$ have Fourier transforms $\widetilde{f}$ and $\widetilde{g}$, respectively. Their convolution is defined by
...
\begin{lem}\label{lem::Poisson}
	(Poisson summation formula) Let $f(\bm{\rho},z)$ be a function that is periodic in $\bm{\rho}$ and non-periodic in $z$. Suppose that $f$ has Fourier transform $\widetilde{f}$ and $\bm{r}=(\bm{\rho},z)$. Then one has
	\begin{equation}
		\sum_{\bm{m}\in \mathbb{Z}^2} f(\bm{r} + \V{\mathcal{M}}) = \frac{1}{2\pi L_x L_y}\sum_{\bm{h}\in \mathcal{K}^2}\int_{\mathbb{R}}\widetilde{f}(\bm{h},\kappa)e^{\m{i} \bm{h}\cdot\bm{\rho}}e^{\m{i} \kappa z}d\kappa.
	\end{equation}
\end{lem}
\begin{lem}\label{lem::2dfourier}
	...
	\begin{equation}
		\widetilde{f}(h,z)=2\pi\int_{0}^{\infty}J_0(h \rho)f(\rho,z)\rho \,d\rho.
	\end{equation}
\end{lem}

\section{The ideal-gas assumption}\label{app::ideal-gas}
Let $\bm{\psi}$ represent a statistical quantity in an interacting particle system, and we aim to analyze its root mean square value given by
\begin{equation}\label{eq::deltaS}
    \delta \bm{\psi}:=\sqrt{\frac{1}{N}\sum_{i=1}^{N}\|\bm{\psi}_{i}\|^2},
\end{equation}
where $\bm{\psi}_{i}$ denotes the quantity associated with particle $i$ (e.g., energy for one dimension or force for three dimensions). Assume that $\bm{\psi}_{i}$ takes the form
\begin{equation}
	\bm{\psi}_{i}=q_{i} \sum_{j \neq i} q_{j} \bm{\zeta}_{i j},
\end{equation}
...
\begin{equation}
	\left\langle\|\bm{\psi}_{i}\|^2\right\rangle=q_{i}^2 \sum_{j, k \neq i} q_{j} q_k\left\langle\boldsymbol{\zeta}_{i j} \boldsymbol{\zeta}_{i k}\right\rangle \approx q_{i}^2 \zeta^2 Q,
\end{equation}
where $Q=\sum_{i=1}^{N}q_i^2$ (note that the total charge vanishes by neutrality). By applying the law of large numbers, one obtains $\delta\bm{\psi}\approx \zeta Q/\sqrt{N}$, which can be utilized for the mean-field estimation of the truncation error.

\section{The Debye--H\"uckel approximation}\label{app::Debye}
Under the Debye--H\"uckel (DH) approximation (also known as the Debye--H\"uckel theory), one is able to estimate functions associated with the $i$-th particle in the form:
\begin{equation}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
where $\kappa=\sqrt{4\pi\beta q^2\rho_r}$ denotes the inverse Debye length $\lambda_{\text{D}}$. By this definition, the net charge density for $r>r_{a}$ is $\rho_>(\bm{r})=-\kappa^2\phi(\bm{r})$. Let us fix $\bm{r}_i$ at the origin. Given these considerations, for $r\geq r_a$, one obtains the following estimate:
...
Let $N$ be the number of particles, $\bm{r}_i=(r_{i,x},r_{i,y},r_{i,z})$, $\bm{v}_i=(v_{i,x},v_{i,y},v_{i,z})$ and $\bm{f}_i=(f_{i,x},f_{i,y},f_{i,z})$ represent the position, velocity and force of the $i$th particle along each of three dimensions, respectively. The $z$-$z$ component of the pressure tensor is computed by the formula
\begin{equation}
