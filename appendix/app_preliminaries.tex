...
\begin{equation}\label{eq::B.1_app}
...
\end{equation}
via the convolution theorem and the Poisson summation formula (see Lemmas~\ref{lem::Convolution} and \ref{lem::Poisson}, respectively). Applying the inverse Fourier transform to Eq.~\eqref{eq::B2_app} yields
\begin{equation}\label{eq::phil_app}
...
\begin{equation}\label{eq::integral_app}
	\int_{\mathbb{R}} \frac{e^{-(h^2+\kappa^2)/(4\alpha^2)}}{h^2+\kappa^2} e^{-\m{i} \kappa z} d\kappa = \frac{\pi}{2h} \left[\xi^{+}(h,z)+\xi^{-}(h,z)\right]
\end{equation}
for $\bm{h}\neq\bm{0}$, where $\xi^{\pm}(h,z)$ are defined via Eq.~\eqref{eq::xi20}. Substituting Eq.~\eqref{eq::integral_app} into the first term of Eq.~\eqref{eq::phil_app} yields $\phi_{\ell}^{\bm{h}}(\bm{r})$ defined via Eq.~\eqref{eq:philk}.

By Theorem~\ref{order}, the zero-frequency term $\phi^{\bm{0}}_{\ell}(z)$ always exists and its derivation is very subtle. Let us apply the 2D Fourier transform (see Lemma~\ref{lem::2dfourier}) to Poisson's equation Eq.~\eqref{eq::B.1_app} only on periodic dimensions, and then obtain
\begin{equation}\label{eq::B.5_app}
...
\end{equation}
The solution of Eq.~\eqref{eq::B.5_app} for $\bm{k}=\bm{0}$ can be written as the form of double integral that is only correct up to a linear mode,
\begin{equation}\begin{split}
	...
	\end{split}
\end{equation}
To analyze the short-range component $\phi_{s}(\bm{\rho},z)$ using a procedure similar to Eqs.~\eqref{eq::B.1_app}-\eqref{eq::integral_app}, one obtains
\begin{equation}\begin{split}
...
% \section{Systems with charged slabs}\label{sec::sysslabs_app}
...
% \begin{equation}\label{eq::PoionWall_app}
...
% \begin{equation}\label{eq::boundionwall_app}
...
% % \begin{equation}\label{eq::chargeneu_app}
...
% \begin{equation}\label{eq::phiion-ion_app}
...
% The 2D Fourier series expansion of~$\phi_{\text{p-s}}$ is provided in the following Theorem~\ref{thm::ionwall_app}, where its convergence rate is controlled by the smoothness of surface charge densities.
% \begin{thm}\label{thm::ionwall_app}
...
% 	\begin{equation}\label{eq::phiionwall_app}
...
% 	\begin{equation}\label{eq::phionwallzero_app}
...
% \end{thm}
% \begin{proof}
% 	For $\bm{k}\neq\bm{0}$, applying the quasi-2D Fourier transform to both sides of Eq.~\eqref{eq::PoionWall_app} yields
% 	\begin{equation}
...
% 	Finally, applying the corresponding inverse transforms to $\widetilde{\phi}_{\text{p-s}}(\bm{k},\kappa)$ and $\widehat{\phi}(\bm{0},z)$ such that the boundary conditions Eq.~\eqref{eq::boundionwall_app} is matched, one has $A_0=B_0=0$. The proof of Eqs.~\eqref{eq::phiionwall_app}-\eqref{eq::phionwallzero_app} is then completed. 
% \end{proof}
...
% \begin{equation}\label{eq:spectial_app}
...
% For completeness, Proposition \ref{welldefinedness_app} provides the result of the well-definedness.
% \begin{prop} \label{welldefinedness_app}
...
% 	By boundary conditions ~\eqref{eq::boundary2} and \eqref{eq::boundionwall_app} and the charge neutrality condition Eq.~\eqref{eq::chargeneu_app}, one has
% 	\begin{equation}
% 		\begin{split}
...
% For the the particle-slab interaction formulation, we observe a constant discrepancy between Eq.~\eqref{eq:spectial_app} derived here and those in literature~\cite{dos2017simulations,yi2017note}. 
...
% And one should use Eq.~\eqref{eq:spectial_app} derived here for correct simulations.
...
% \begin{align}\label{eq::34_app}
% 	U = U_{\text{p-p}} + U_{\text{p-s}}, \quad \text{with} \quad U_{\text{p-p}} := U_{s} + \sum_{\bm{k}\neq\bm{0}}U_{\ell}^{\bm{k}}+U_{\ell}^{\bm{0}}- U_{\text{self}}\;,
% \end{align}
% where $U_*=\sum_{i}\phi_*$ with $*$ representing any of the subscripts used in Eq.~\eqref{eq::34_app}.
...
\begin{equation}\label{eq::delta^2phi_app}
...
\begin{equation}\label{eq::asyerfc_app}
	\erfc(r)=\frac{e^{-r^2}}{\sqrt{\pi}}\sum_{m=0}^{\infty}(-1)^{m}\left(\frac{1}{2}\right)_mz^{-(2m+1)}
\end{equation}
as $r\rightarrow \infty$, where $(x)_m=x(x-1)\cdots(x-m+1)=x!/(x-m)!$ denotes the Pochhammer's symbol. Substituting Eq.~\eqref{eq::asyerfc_app} into Eq.~\eqref{eq::delta^2phi_app} and truncating at $m=1$ yields Eq.~\eqref{eq::Qs}. 
