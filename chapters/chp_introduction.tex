...
Confined quasi-two-dimensional (quasi-2D) Coulomb systems~\cite{mazars2011long}, which are macroscopic in two dimensions but with atomic-size thickness in the other, have attracted much attention in studies of magnetic and liquid crystal films, supercapacitors, crystal phase transitions, dusty plasmas, ion channels, superconductive materials, and quantum devices~\cite{kawamoto2002history, hille2001ionic, teng2003microscopic, Messina2017PRL, mazars2011long, saito2016highly, liu20192d}. 
Typically, such systems possess a nano-sized longitudinal thickness in the $z$ direction, achieved through confinement, while remaining bulk-like and modeled as periodic in the transverse $xy$ directions, hence endowed with an inherent multi-scale nature.
...
In these systems, substrate materials used for nanoscale confinement can range from dielectric to metallic, and more recently electromagnetic metamaterials, which have been developed with permittivities that can take negative values~\cite{veselago1967electrodynamics, smith2004metamaterials, cheng2017tunable, xie2022recent, xu2020polyaniline} when excited by electromagnetic waves of specific frequencies.  
...
Interestingly, even for water, the perpendicular component of its tensorial dielectric function has been observed to be negative within sub-Angstrom distances from the surface by nanoconfinement~\cite{Kornyshev1996Static, Schlaich2016Water, Kornyshev2021Nonlocal}.
...
For numerical simulations of quasi-2D Coulomb systems, it is of great importance to evaluate lattice kernel summations in the form of
\begin{equation}\label{eq::phi12}
	\phi(\V x) = \sum_{\V{m}} \sum_{j = 1}^{N} \rho_j K(\V{x} - \V{y}_j + \V{m} \circ \V{L})\;,
\end{equation}
where~$\V{x}, \V{y}_j \in \mathbb{R}^3$ are $3$-dimensional vectors in a rectangular box $\Omega$ with $\V{L}$ the vector of its edge lengths, $\rho_j$ refers to the density or weight, $\V{m} \in \mathbb{Z}^2 \times \{0\}$ exerts periodicity in the first $2$ directions, ``$\circ$'' represents the Hadamard product, and~$K(\V{x})$ is the kernel function whose form depends on the physical problem of interest. 
For homogeneous systems without dielectric confinement, the kernel function is given by $K(\V{x}) = \abs{\V{x}}^{-1}$.
...
For quasi-2D systems,  numerous methods have also been developed over the past decades, mostly relying on modifications to the FMM and FFT-based Ewald methods to achieve $O(N\log N)$ or even $O(N)$ complexity.
...
Instead of exactly solving the quasi-2D problem, alternative approaches approximate the quasi-2D summations by 3D periodic summations with correction terms, such as the Yeh--Berkowitz (YB) correction~\cite{yeh1999ewald} and the electric layer correction (ELC)~\cite{arnold2002electrostatics}.
...
The methods mentioned above encounter challenges when addressing sharp dielectric interfaces. In recent years, several strategies have been developed to effectively simulate quasi-2D systems under dielectric confinement.
A common approach combines the Ewald2D summation with the image charge method (ICM)~\cite{jackson1999classical,frenkel2023understanding}, in which the polarization effects induced by dielectric planar interfaces are modeled through an infinite series of reflected image charges in homogeneous space.
...
\rev{
	...
	This approach is very flexible and can be applied to various geometries, but it is computationally expensive due to the need to solve a large linear system.
}
...
When combined with the FMM~\cite{greengard1987fast,kohnke2020gpu} or the fast Fourier--Chebyshev transform~\cite{trefethen2000spectral}, these methods can achieve computational complexities of $ O(N\log N)$ or even $ O(N)$.
...
A few challenges remain. First, FFT-based methods need extra techniques to properly handle the non-periodic direction, such as truncation~\cite{parry1975electrostatic}, regularization~\cite{nestler2015fast}, or periodic extension~\cite{lindbo2012fast}, which may lead to algebraic convergence or require extra zero-padding to guarantee accuracy. 
Recent advancements by Shamshirgar \emph{et al.}~\cite{shamshirgar2021fast}, combining spectral solvers with the truncated kernel method (TKM)~\cite{vico2016fast}, have reduced the zero-padding factor from $6$ to $2$~\cite{lindbo2012fast}, which still requires doubling the number of grids with zero-padding. 
...
We provide a comprehensive theoretical analysis, including rigorous error estimates and complexity bounds, which are thoroughly validated through extensive numerical experiments.
...
The first part focuses on the theoretical analysis of long-range interactions in quasi-2D Coulomb systems.
...
In Chapter~\ref{chp_icmewald2d}, we first introduce the Ewald summation method for dielectric-confined quasi-2D Coulomb systems, focusing on the ICM-Ewald2D method and its reformulation.
...
Then, based on the SOE approximation, we propose an algorithm named the sum-of-exponential Ewald2D (SOEwald2D) method, which utilizes the SOE approximation and the random batch sampling method to accelerate the Ewald2D summation for quasi-2D Coulomb systems without dielectric confinement.
Numerical results are provided to validate the accuracy and efficiency of the proposed method.

In Chapter~\ref{chp_rbe2d}, we focus on the quasi-2D Coulomb systems with dielectric confinement.
...
Then, we introduce a novel algorithm called the random batch Ewald2D (RBE2D) method, which efficiently handles quasi-2D Coulomb systems with dielectric confinement while maintaining excellent scalability for large-scale simulations on modern supercomputing architectures.
Detailed numerical results are provided to validate the accuracy and efficiency of the proposed method.

In Chapter~\ref{chp_quasiewald}, we investigate quasi-2D systems with negative dielectric confinement, where the polarizable materials possess negative dielectric constants, which results in failure of ICM-based methods.
...
We show that with proper singularity subtraction, the QEM can be extended to handle negative dielectric confinement.
