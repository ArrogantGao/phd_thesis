...
\begin{proof}
	...
	\begin{equation}
		...
		\end{split}
	\end{equation}
	Here, the summation over Fourier modes is approximated by an integral over the reciprocal space, and one chooses a specific $(h,\theta,\varphi)$ so that the coordinate along $\cos\theta$ of $\bm{h}$ is in the direction of a specific vector $\bm{r}$, and $\bm{h}\cdot\bm{r}=h r \cos\varphi$.
