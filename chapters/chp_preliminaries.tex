...
The central simulation cell $\Omega\subset\Omega_{\text{c}}$ has side lengths~$\bm{L}=(L_x, L_y, H)$ and contains $N$ particles located at $\{\V{r}_i\}_{i=1}^{N}$ with charges $\{q_i\}_{i=1}^{N}$. 
...
\begin{equation}\label{eq:sandwich}
    ...
    \end{array} \right.
\end{equation}
where~$\eps_{\T u}$,~$\eps_{\T c}$ and~$\eps_{\T d}$ are all positive constants, whose specific values depend on material properties inside each layer. 
...
We consider a lattice $\Lambda(\mathcal{S},R)=\{\Omega(\bm{m})|\bm{m}/R\in\mathcal{S}\}$ with $R\in\mathbb{R}$ being a truncation parameter. 
...
\begin{prop}\label{prop::boundary}
	If the series summation of $\phi(\bm{r})$ in Eq.~\eqref{eq::pairssum} satisfies both conditions stated in Theorem~\ref{order}, then it is a unique solution to Poisson's equation Eq.~\eqref{eq::Poisson} given the far-field boundary condition
...
\begin{proof}
	...
	\end{equation} 
	Poisson's summation formula (see Appendix~\ref{app::Fourier}) indicates 
...
Next, we study the uniqueness of the solution of Poisson's equation under DPBCs and Eq.~\eqref{eq::boundary1}. Suppose there exist two solutions $\phi_1(\bm{r})$ and $\phi_2(\bm{r})$,
...
\begin{equation}
    \left\{
    \begin{array}{ll}
        - \grad_{\V{r}} \left[ \eps(\V{r}) \grad_{\V{r}} G(\V{r},~\V{r}^\prime) \right] = \d (\V{r} - \V{r}^\prime) & \V{r} \in \mathbb{R}^3 \;, \\
        ...
        G(\V{r},~\V{r}^\prime) \to 0 & \text{as}~\abs{\V{r}} \to \infty\;,
    \end{array}
    \right.
    \label{eq:Poisson_G}
\end{equation}
...
$\gamma_{+}^{(l)} = \gamma_{\T u}^{\lceil l/2 \rceil} \gamma_{\T d}^{\lfloor l/2 \rfloor}$, $\gamma_{-}^{(l)} = \gamma_{\T d}^{\lceil l/2 \rceil} \gamma_{\T u}^{\lfloor l/2 \rfloor}$, $\V{r}_{+}^{\prime(l)} = (x^\prime, y^\prime, z_{+}^{(l)})$, and $\V{r}_{-}^{\prime(l)} = (x^\prime, y^\prime, z_{-}^{(l)})$ are the scaling factors and positions of the $l$-th level image charges.
...
\begin{equation}\label{eq:z_l}
    z_{+}^{(l)} = (-1)^l z^\prime + 2\lceil l/2\rceil H \;,\quad z_{-}^{(l)} = (-1)^l z^\prime - 2\lfloor l/2\rfloor H \;.
\end{equation}
Assuming all dielectric constants are positive, it follows that $\abs{\gamma_{\mrm{u}}},\abs{\gamma_{\T d}}< 1$. 
...
However, as mentioned in Chapter~\ref{chp_intro}, recent studies have shown that special materials~\cite{Kornyshev1996Static, Schlaich2016Water, Kornyshev2021Nonlocal} can have negative dielectric constants, which may lead to the divergence of the image charge series.
...
We then introduce \rev{its} applications to both fully and doubly periodic systems, i.e. the Ewald3D and Ewald2D summation, respectively.
...
\begin{align}
	...
	U_{\ell} & = \frac{2 \pi}{L_x L_y L_z} \sum_{\V{k} \neq \V{0}} \frac{1}{k^2} \abs{\rho(\V{k})}^2 e^{-\frac{k^2}{4\alpha^2}} - \frac{\alpha}{\sqrt{\pi}}\sum_{i=1}^{N}q_i^2\;,
\end{align}
...
\end{equation}
In practice, both summations in real and reciprocal spaces are truncated at a cutoff radius $r_c$ and $k_{c}$, respectively.
A common choice is to set $r_c = s / \alpha$ and $k_c = 2 s \alpha$, where $s$ is a parameter to control the accuracy of the truncation.
It has been shown that under such truncation, the truncation error of total energy decays as $O(e^{-s^2} / s^{2})$, and time complexity of Ewald3D summation is $O(N^{1.5})$.
...
Recently, this idea has been applied to fully-periodic Lennard-Jones and Coulomb systems~\cite{liang2021random, jin2021random, liang2023SISC, chen2025random}, demonstrating superscalability in large-scale simulations~\cite{liang2022superscalability, liang2022improved,gao2024rbmdmoleculardynamicspackage}. 
...
The SPMF originates from the local molecular field theory for Coulomb systems~\cite{chen2006local, hu2010efficient}, which states that algorithms must share a mean-field property, that is
...
\begin{equation}
	U_{\ell}^{\V{k}} = \frac{2 \pi}{L_xL_yL_z} \sum_{\V{k} \neq \V{0}} \frac{1}{k^2} \abs{\rho(\V{k})}^2 e^{-\frac{k^2}{4\alpha^2}}\;.
\end{equation}
...
\begin{equation}
	\mathcal{P}_{\V{k}} = \frac{e^{-\frac{k^2}{4\alpha^2}}}{S},~~\V{k} \neq \V{0} \;,
\end{equation}
where $S$ is the normalization constant:
\begin{equation}
	S = \sum_{\V{k} \neq \V{0}} e^{-\frac{k^2}{4\alpha^2}}\;.
\end{equation}
Thus in RBE, the long-range component is approximated as follows:
\begin{equation}
	U_{\ell} \approx U_{\ell, *} =  \frac{2 \pi S}{L_xL_yL_z\,P} \sum_{\eta = 1}^{P} \frac{1}{k_{\eta}^2} \abs{\rho(\V{k}_{\eta})}^2 - \frac{\alpha}{\sqrt{\pi}}\sum_{i=1}^{N}q_i^2\;,
\end{equation}
where $\{\V{k}_{\eta}\}_{\eta=1}^{P}$ are $P$ random vectors sampled from the discrete Gaussian distribution~$\mathcal{P}_{\V{k}}$, thus $U_{\ell, *}$ is an unbiased estimator of $U_{\ell}$.
The same procedure is also valid for the force calculation.
It has been proved that the batch size $P$ is an $O(1)$ constant independent of the system size $N$, so that the time complexity of RBE is always $O(N)$.


\subsection{Ewald2D summation} \label{sec::ewald2d}

Throughout the remaining sections, we will extensively use Fourier transforms for the DPBCs, which leads to the well-known Ewald2D~\cite{parry1975electrostatic, heyes1977molecular, de1979electrostatic} formula for quasi-2D Coulomb systems.
For ease of discussion, the mathematical notations and definitions are first provided.

\begin{defn}\label{Def::Fourier}(Quasi-2D Fourier transform) 
...
\begin{proof}
	...
	\end{equation}
	for $\bm{h}\neq\bm{0}$, where $\xi^{\pm}(h,z)$ are defined via Eq.~\eqref{eq::xi_def}. Substituting Eq.~\eqref{eq::integral} into the first term of Eq.~\eqref{eq::phil} yields $\phi_{\ell}^{\bm{h}}(\bm{r})$ defined via Eq.~\eqref{eq:philk}.
	...
	\begin{equation}\begin{split}
			\phi_{s}^{\bm{0}}(z) =& \frac{\pi}{L_x L_y} \sum_{j=1}^{N} q_{j} \lim_{h\rightarrow 0} \frac{1}{h} \left[2e^{-h|z-z_{j}|}-\xi^{+}(h,z-z_{j}) - \xi^{-}(h,z-z_{j})\right]\\
	...
	\begin{equation}
		A_0 = \frac{2\pi}{L_xL_y}\sum_{j=1}^{N}q_{j} \equiv 0,\quad \text{and}\quad B_0=-\frac{2\pi}{L_xL_y}\sum_{j=1}^{N}q_{j}z_{j}.
	\end{equation}
...
\begin{equation}\label{eq::Ul_Ewald2D}
    U_{\ell} =  \frac{\pi}{2L_xL_y}\sum_{i, j=1}^N  q_iq_{j} \sum_{\V h\neq \V 0}\frac{e^{\i \V h\cdot \V \rho_{ij}}}{h}\mathcal{G}_{\alpha}(h,z_i - z_{j})  - \frac{\alpha}{\sqrt{\pi}}\sum_{i=1}^{N}q_i^2+\mathcal{J}_0\;.
\end{equation}
...
In practice, truncation in real and Fourier spaces is performed simultaneously, and the truncation error is clearly configuration dependent.
...
Details of the ideal-gas assumption are summarized in Appendix~\ref{app::ideal-gas}.
...
In the following analysis, we denote the cutoff radii in real and Fourier spaces as $r_c$ and $k_c$, i.e., one only calculates the terms satisfying $\abs{\bm{r}_{ij} + \mathcal{M}} \leq r_c$ and 
...
\begin{proof}
	...
	\end{equation}
	for $i$th particle, which involves neglecting interactions beyond $r_c$. By the analysis in Appendix~\ref{app::ideal-gas}, this part of error can be approximated by $\delta\mathscr{E}_{\phi_{s}}$ with 
	...
	\end{equation}
	where $\mathscr{Q}_{\emph{s}}(\alpha,r_c)$ is defined via Eq.~\eqref{eq::Qs2}. Note that the $\erfc(r)$ function satisfies \cite[pp.~109--112]{olver1997asymptotics}
	\begin{equation}\label{eq::asyerfc}
		\erfc(r)\sim\frac{e^{-r^2}}{\sqrt{\pi}}\sum_{m=0}^{\infty}(-1)^{m}\left(\frac{1}{2}\right)_m r^{-(2m+1)}
	\end{equation}
	as $r\rightarrow \infty$, where $(x)_m=x(x+1)\cdots(x+m-1)=\Gamma(x+m)/\Gamma(x)$ denotes the Pochhammer symbol. Substituting Eq.~\eqref{eq::asyerfc} into Eq.~\eqref{eq::delta^2phi} and truncating at $m=1$ yields Eq.~\eqref{eq::Qs}. 
	...
	\end{equation}
	For a large $k_c$, one can safely replace the truncation condition with $\sqrt{h^2+\kappa^2}>k_c$, resulting in
	\begin{equation}
		\begin{split}
		...
		\end{split}
	\end{equation}
	Here, the summation over Fourier modes is approximated by an integral over the reciprocal space, and one chooses a specific $(h,\theta,\varphi)$ so that the coordinate along $\cos\varphi$ of $\bm{h}$ is in the direction of a specific vector $\bm{r}$, and $\bm{h}\cdot\bm{r}=h r \cos\varphi$.
...
\begin{rmk}
	...
	\begin{equation}\label{eq:truncation_error}
		\mathscr{E}_{\phi_s}(r_c, \alpha)\approx \mathscr{E}_{\phi_\ell}(k_c, \alpha) \sim \sqrt{Q} 
...
\begin{itemize}
	...
	\item The function $\xi^{\pm}(h, z_{ij})$ is ill-conditioned: It grows exponentially as $h z_{ij}$ grows, leading to catastrophic cancellation in actual computations with prescribed machine precision.
\end{itemize}
...
In the presence of charged slabs, boundary layers naturally arise -- opposite ions accumulate near the interface, forming an electric double layer. The structure of electric double layers plays an essential role in the properties of interfaces and has attracted much attention~\cite{messina2004effect,breitsprecher2014coarse,moreira2002simulations}. Since charges on the slabs are often represented as a continuous surface charge density, we present the Ewald2D formulation with which such a situation can be well treated.
...
\begin{equation}\label{eq::boundionwall}
	\lim_{z\rightarrow\pm\infty}\phi_{\text{p-s}}(\bm{r})=-\lim_{z\rightarrow\pm\infty} \frac{2\pi}{L_xL_y}\left(\int_{\mathcal{R}^2}\sigma_{\mathrm{bot}}(\bm{\rho})|z|d\bm{\rho}+\int_{\mathcal{R}^2}\sigma_{\mathrm{top}}(\bm{\rho})|z-L_z|d\bm{\rho}\right)
\end{equation}
...
\end{equation}
with each component given by Eqs.~\eqref{eq:phi_s}, \eqref{eq:philk}, \eqref{eq:phil0}, and \eqref{eq::self}, respectively. 
...
\begin{proof}
	...
	\end{equation}
	For $\bm{h}=0$, one first applies the 2D Fourier transform in $xy$ to obtain
	\begin{equation}
...
Here zero is retained to indicate the location of the bottom slab. 
...
\begin{prop} \label{welldefinedness}
	The total electrostatic potential $\phi$ is well-defined, \rev{i.e., satisfies the infinite boundary condition that the potential is a constant as $z\rightarrow \pm \infty$.}
\end{prop}
\begin{proof}
...
For the particle-slab interaction formulation, we observe a constant discrepancy between Eq.~\eqref{eq:spectial} derived here and those in literature~\cite{dos2017simulations,yi2017note}. 
...
\end{align}
where $U_*=\frac{1}{2}\sum_{i}q_{i}\phi_*$ with $*$ representing any of the subscripts used in Eq.~\eqref{eq::34}.
