...
\end{figure}
Throughout this chapter, we will extensively use 2D-Fourier transforms, whose definition is given by
\begin{defn}
...
Then by employing the Dirichlet to Neumann mapping technique to the Poisson's equation Eq.~\eqref{eq:Poisson_G}, we can derive the Green's function~$G(\V{r}, \V{r}_0)$ in the reciprocal space. 

\begin{thm}\label{Proposition...GreensFunction}
    As we define
    \begin{equation}
        \Hat{G}(\V{h}, z; z_0) = \int_{\mathbb{R}^2} G(\V{r}, \V{r}_0) \exp{(-\mathrm{i} \V{h} \cdot \V{\rho})} d \vrho\;,
    \end{equation}     
then for $\V{h}=\vzero$,   
...
therefore, we have 
\begin{align*}
    \partial_z\Hat{G}_{\mrm{u}}(\V{h}, z; z_0)&=-h\Hat{G}_{\mrm{u}}(\V{h}, z; z_0),~z > H\;,\\
     \partial_z\Hat{G}_{\mrm{d}}(\V{h}, z; z_0)&=h\Hat{G}_{\mrm{d}}(\V{h}, z; z_0),~z < 0\;,
\end{align*}
...
\end{align*} 
Finally, for $z\in[0, H]$, $\Hat{G}_{\mrm{c}}(\V{h}, z; z_0)$ takes the form
\begin{equation*}
...
\begin{rmk}\label{Remark...Nonzero+larger}
    ...
    However, for negatively confined systems with $\gamma_u \gamma_d > 1$, a first-order pole exists at $h_0 = \ln(\gamma_{\mrm{u}} \gamma_{\mrm{d}})/(2H)$ and the Green's function diverges.
...
\end{equation}
and we  shall decompose the Green's function  $ G(\V{r}, \V{r}_0)$ correspondingly into
\begin{equation}
    G(\V{r}, \V{r}_0)= G_s(\V{r}, \V{r}_0)+  G_\ell(\V{r}, \V{r}_0)\;,
\end{equation}
where $ G_s(\V{r}, \V{r}_0)$ and $  G_\ell(\V{r}, \V{r}_0)$ are the solutions to the Poisson's equation Eq.~\eqref{eq:Poisson_G} except that $\delta(\V{r}-\V{r}_0)$ is replaced respectively by $\sigma_s  (\V{r};\V{r}_0)$ and $\sigma_\ell  (\V{r};\V{r}_0)$.
\begin{corollary}
...
\end{equation*} 
respectively.
Consequently, the Green's function $ G_s(\V{r}, \V{r}_0)$ and $  G_\ell(\V{r}, \V{r}_0)$ can be recovered following the inverse Fourier transform of $  \hat{G}_s(\V{h}, z; z_0)$ and $  \hat{G}_\ell(\V{h}, z; z_0)$,
\begin{equation*}
\begin{aligned}
 G_s(\V{r}, \V{r}_0)&=   \frac{1}{4\pi^2}\int_{\sR^2} \hat{G}_s(\V{h}, z; z_0)\exp{(\mathrm{i} \V{h} \cdot \V{\rho})} d \V{h}\\
 &= \frac{1}{4\pi^2}\int_{\sR^2} \hat{G}(\V{h}, z; z_0)\left[1-\exp\left(-\frac{h^2}{4\alpha}\right)\right]\exp{(\mathrm{i} \V{h} \cdot \V{\rho})} d \V{h}\;,\\
  G_\ell(\V{r}, \V{r}_0)&=   \frac{1}{4\pi^2}\int_{\sR^2} \hat{G}_\ell(\V{h}, z; z_0)\exp{(\mathrm{i} \V{h} \cdot \V{\rho})} d \V{h}\\
...
In actual calculations, due to the short range nature of~$G_s(\cdot, \V{r}_j)$, the infinite sum in Eq.~\eqref{eq::U_s} is truncated to a finite sum in real space with a cutoff $r_c$, given by
\begin{equation}\label{eq::U_s_truncated}
    U_s \approx U_{s, *} = \frac{1}{2}\sum_{\abs{\bm{\rho}_{ij, m}} \leq r_c} q_i q_j G_s(\V{r}_i+\vL_m, \V{r}_j)\;,
\end{equation}
...
\begin{equation*}
 \int_0^{+\infty} F(h)d  h=  \int_0^{h_c} F(h)d  h+\int_{h_c}^{+\infty} F(h)d  h\;,
\end{equation*} 
...
\end{equation}
using the Gauss-Laguerre or Gauss-Hermite quadrature, and the Gauss-Legendre quadrature, respectively.
First for a fixed~$\rho = 50$, as shown by Fig.~\ref{fig:Gauss_int} (a) and (b), the truncated Gauss-Legendre formula on interval~$[0, 36]$ or~$[-6, 6]$ converges much more rapidly as the number of points~$n$ increases compared to the Gauss-Laguerre or Gauss-Hermite formula.
Then for varying~$\rho \in [0, 10]$, we set the desired accuracy~$\mathcal{E}_r = 10^{-4}$, $10^{-8}$ and $10^{-12}$, and compute the number of points needed for the truncated Gauss-Legendre formula to reach the given accuracy~$\mathcal{E}_r$.
As shown in Fig.~\ref{fig:Gauss_int_n} (a) and (b), in both cases the integrals converge rapidly and only take a few tens of points to reach the accuracy.
In this way, both~$I_1$ and~$I_2$ are transformed into integrals on a finite region and can be evaluated easily using the Gauss–Legendre formula.

\begin{figure}[htbp]
    \centering
    \includegraphics[width = \linewidth]{figs/integral_n.pdf}
    \caption{
        The relative error of numerical evaluation of integrals (a) Eq.~\eqref{eq:I1} via Gauss–Laguerre quadrature and (b) Eq.~\eqref{eq:I2} via Gauss–Hermite quadrature on an infinite interval or Gauss-Legendre quadrature on a truncated interval, respectively;~$n$ is the order of the quadrature.
    }
    \label{fig:Gauss_int}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{
    Order of quadrature~$n$ needed to numerically evaluate the integrals (a) Eq.~\eqref{eq:I1} and (b) Eq.~\eqref{eq:I2} to reach the given relative accuracy~$\mathcal{E}_r$ using the Gauss-Legendre quadrature on a truncated interval~$[0, k_{c1}]$ and~$[-k_{c2}, k_{c2}]$, respectively.
    }
...
\end{equation}
and for simplicity, we denote hereafter that 
...
\begin{align*}
    S_0 
    & = \frac{1}{2 L_xL_y} \sum_{i = 1}^N q_i \left( \sum_{j = 1}^{i }{q_j (z_i - z_j)} \right) \\
    & = \frac{1}{2 L_xL_y} \sum_{i = 1}^N q_i \left( z_i \sum_{j = 1}^{i }q_j - \sum_{j = 1}^{i }z_j q_j  \right) \;.
\end{align*}
...
\[
S_0= \frac{1}{2 L_xL_y} \sum_{i = 1}^N q_i \left( z_iT_1(i)-T_2(i)\right)\;,
\]
which requires only 
...
\begin{equation}\label{eq::hk_qem}
...
% hence
%
% \begin{equation}\label{eq::Happ_qem}
...
\begin{equation}\label{eq::RBapp_qem}
 U_\ell^{\V{h}\neq \vzero} \approx U_{l,\ast}^{\V{h}\neq \vzero}  :=   \frac{S}{P}\sum_{\eta=1}^P \phi(\V{h}_{\eta})=  \frac{S}{P}\sum_{\eta=1}^P \sum_{q=1}^4S_q(\V{h}_{\eta})\beta_q(\V{h}_{\eta})\;,
\end{equation} 
...
\begin{equation}\label{eq::xichi_qem}
%	\Xi :=U_{l,\ast}^{\V{h}\neq \vzero}-U_\ell^{\V{h}\neq \vzero},\quad\text{and}\quad
    \V{\chi}_{i} :=\vF_{l,i}^*-\vF_{l,i}=\grad_{\V{r}_i} U_{l,\ast}^{\V{h}\neq \vzero}-\grad_{\V{r}_i} U_{l}^{\V{h}\neq \vzero}\;.
\end{equation}
Proposition~\ref{prop:unbiased_qem} is obtained directly by the definition of the importance sampling.
\begin{prop}\label{prop:unbiased_qem}
...
\begin{align*}
        ...
        &~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-\frac{1}{S} \sum_{\bm{h}_2\in\fK^2, \bm{h}_2\neq \vzero}\nabla_{\bm{r}_i}\phi(\V{h}_2)\exp\left(-\frac{\Norm{\V{h}_2}^2}{4\alpha}\right)\Bigg|\Bigg|^2\;.
\end{align*}
\end{prop}
% Furthermore, under the Debye-H$\ddot{\text{u}}$ckel approximation~\cite{levin2002electrostatic}, we have  the following~Lemma~\ref{lem::upper_bound_phiRB_qem}, whose proof can be found in Appendix G in~\cite{gan2024fast}.
% \begin{lem} \label{lem::upper_bound_phiRB_qem}
...
Finally, we introduce the following Theorem~\ref{thm:unbiased_qem} for the boundedness and convergence in the fluctuations originating from the random batch approximation.
\begin{thm}\label{thm:unbiased_qem}
	Under the assumption of the DH theory, the variance of the force estimators has closed upper bounds
	\begin{equation}
		  \mathbb{E}\Norm{\V{\chi}_{i}}^2\leq\frac{ S^2q_i^2\lambda_{D}^4}{2P L_x^2L_y^2}\left( \frac{1+\Abs{\gamma_{\mrm{u}}}+\Abs{\gamma_{\mrm{d}}}+\Abs{\gamma_{\mrm{u}}\gamma_{\mrm{d}}}}{1-\max\{0,\gamma_{\mrm{u}}\gamma_{\mrm{d}}\}}\right)^2\;,
	\end{equation}
     where~$\lambda_{D}$  represents the Debye length.
\end{thm}
\begin{proof}
We observe that 
\begin{align*}
 \nabla_{\bm{r}_i}\phi(\V{h})&=   \sum_{q=1}^4 \nabla_{\bm{r}_i}\left\{S_q(\V{h})\right\}\beta_q(\V{h})\;,
\end{align*}
thus we have,
%
% \begin{align*}
...
\end{align*}
on the basis of the Debye--H\"uckel estimate (see Appendix~\ref{app::Debye}), the upper bound of~$\abs{\grad_{\V{r}_i} \phi(\V{h})}$ can be estimated as follows 
\begin{equation}
...
\begin{align*}
    ...
    &= \frac{ S^2q_i^2\lambda_{D}^4}{2P L_x^2L_y^2}\left( \frac{1+\Abs{\gamma_{\mrm{u}}}+\Abs{\gamma_{\mrm{d}}}+\Abs{\gamma_{\mrm{u}}\gamma_{\mrm{d}}}}{1-\max\{0,\gamma_{\mrm{u}}\gamma_{\mrm{d}}\}}\right)^2\;.
\end{align*}
\end{proof}
...
Theorem~\ref{thm:unbiased_qem} has demonstrated that the variance of force scales as $\fO\left(\frac{1}{P}\right)$, unaffected by the growth of the system size $N$, provided the same Debye length $\lambda_{D}$. 
...
\end{equation}
where~$\varrho$ represents the surface density, so that the cost of the short-range interaction becomes
\begin{equation}
    C_s \propto  N\;.
\end{equation}
In this way, the total cost of the QEM is $O(N)$.
...
To avoid the divergence, we calculate the principal value by splitting the integrand into two parts, given by
...
\begin{equation}
    \delta I < \int_{k_f}^{\infty} \exp{(-a h)} dh = \frac{\exp{(-a k_f)}}{a}\;.
\end{equation}

The integral of the singular part can be calculated analytically, given by
\begin{equation}
    \begin{split}
...
\end{equation}
which directly subtracts the 1st order pole but takes a high order of the Gaussian quadrature to reach high accuracy.

It is also shown that the integral converges much faster with the order of the quadrature.
We set~$a = b = k_0 = H = 1$, and $k_f = 10$,~$20$ and~$30$, the relative errors are shown in Fig.~\ref{fig:error} as functions of the order of Gaussian quadrature, which show that the error decays rapidly to desired accuracies.

\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.5\linewidth]{figs/int_convergence.pdf}
    \caption{Relative error of the numerical evaluation of Eq.~\eqref{eq::int_I} with $a = b = H = k_0 = 1$, as a function of cutoff~$k_f$ and order of the quadrature~$n$.}
    \label{fig:error}
\end{figure}

In summary, the singularity subtraction technique introduced above provides an efficient method for handling the singularity in the short-range interaction by splitting the integrand into smooth and singular parts that can be evaluated separately with high accuracy.
Thus, when combined with this technique, QEM enables the simulation of negatively confined Coulomb systems with high accuracy and efficiency.

\section{Numerical results}
\label{sec:result}

In this section, we validate the accuracy and efficiency of our proposed method by applying it to several typical scientific problems, some of the results are compared with those of previously proposed methods, such as the Ewald2D method, the image charge method, and the HSMA method.
The runtime of QEM in one of the typical scenarios is also tested.
...
We first consider the electrostatic interaction between a pair of charged particles, and the results are compared with those of the ICM-Ewald2D method.
We focus on two simple situations,~$\gamma_{d} = \gamma_{u} = \gamma = \pm 0.95$.
The size of the simulation box is set as~$(100, 100, 50)$, and the particle with charge~$+1$ is fixed at~$(50, 50, 1)$, while the other one with charge~$-1$ is moved along the $x$ axis or the $z$ axis.
...
\begin{figure}[htb]
    ...
    \caption{
        Electric field in (a) $x$ and (b) $z$ between a pair of oppositely charged particles, which are confined in a box of size $(100, 100, 50)$.
        One of the particles is located at $(50, 50, 1)$, and the other one is located at (a) $(50 + \Delta x, 50, 1)$ and (b) $(50, 50, \Delta z)$.
...
In Fig.~\ref{fig:Error_E}, the relative error of the interaction energy is plotted as a function of the parameter~$E$ and the reflection rate~$\gamma$.
...
\begin{figure}[htb]
    ...
    \caption{
        Relative error of the energy of $100$ charged particles randomly distributed in a box of size~$(100, 100, 50)$, as functions of the parameter~$E$ and reflection rates~$\gamma$.
    }
    \label{fig:Error_E}
\end{figure}
...
% The ions are confined by purely repulsive shifted-truncated LJ walls ($\eps_\text{ion-wall} = k_B T$; $\tau_\text{ion-wall} = 0.5 \tau_0$) at $z = 0$ and $z = H$. 
...
In all simulations, we use the reduced unit and set~$L_x = L_y = 100 \tau_0$, and~$H = 50 \tau_0$.
...
The walls are purely repulsive shifted-truncated LJ walls ($\eps_\text{ion-wall} = k_B T$; $\tau_\text{ion-wall} = 0.5 \tau_0$) at $z = 0$ and $z = H$.
...
The system contains 218 cations and 218 anions carrying charge~$\pm e_0$, respectively.
To validate, we compare the charge distribution obtained by QEM with the result of the HSMA~\cite{liang2020harmonic}.
The ion density profiles by QEM and HSMA are shown in Fig.~\ref{fig:MD_qem}, which shows that distributions obtained by QEM and HSMA overlap within statistical accuracy. 
...
Due to the confinements in~$z$, the MSD in the non-periodic dimension shown in Fig.~\ref{fig:msd}(b) behaves differently as a sub-diffusion process, where the MSD stops growing at long times, indicating the ions are confined by the dielectric interfaces.
...
\begin{figure}[htbp]
    ...
    \caption{
        The MSD of the ions in (a)~$xy$ and (b)~$z$, respectively, as functions of simulation time and reflection rates.
    }
    \label{fig:msd}
\end{figure}
...
For QEM, the same parameters as in the previous simulations are applied; for the Ewald2D method, the parameters are selected so that the results are of 4-digit precision.
...
\begin{figure}[htb]
    ...
    The blue and red dots correspond to the CPU time of the Ewald2D method and QEM, and the dashed lines are fitted with slopes of~$2$ and~$1$, respectively.
    }
    \label{fig:timecost}
\end{figure}

\subsubsection{MD simulations of asymmetric systems}
As mentioned above, QEM can be used to simulate asymmetric thin systems.
...
The spatial distribution of ions in~$z$ is shown in Fig.~\ref{fig:non_sym}.
Both cations and anions are attracted towards the lower dielectric plate with a negative reflection coefficient, while being repelled by the upper dielectric plate with a positive dielectric coefficient, thereby forming a strongly asymmetric spatial distribution as illustrated in Fig.~\ref{fig:non_sym}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.50\linewidth]{figs/non_symm.pdf}
    \caption{
        Ion density in~$z$ for electrolytes containing 218 cations and 218 anions, confined by neutral dielectric substrates with asymmetric dielectric mismatch $\gamma_u = 0.95$,~$\gamma_d = -0.95$ at~$z = 0$ and~$10\tau_0$.
    }
    \label{fig:non_sym}
\end{figure}
...
In that simulation, we keep~$\gamma_u$ and $\gamma_d$ equal to~$\gamma = \pm 0.95$, but simulate a $3:1$ salt with $109$ trivalent cations and $327$ monovalent anions. 
...
For cations, due to the increase in the charge of the cations, the repulsive/attractive effects on both sides of the medium are significantly enhanced when~$\gamma$ is greater or smaller than~$0$, respectively.
...
These cases indicate that QEM can also provide significant advantages in handling systems with particle asymmetry, thinness, and strong polarization effects.
...
At short distance ($\tau_0<\Delta x < 10\tau_0$), we observe from Fig.~\ref{fig:force_x}(a) a continuous transition in the near-field interaction from like-charge attraction (LCA) into repulsion as~$\gamma$ increases from $-10$ to $+10$,
...
\begin{equation}
    \sigma(\V{r}) = \lim_{z \to 0^+} \nu \ell_B \eps_0  \left( 1 - \frac{\eps}{\eps'} \right) \partial_z G(\V{r}, \V{r}_0)\;,
\end{equation}
...
The oscillatory field lines have a very similar structure to that of a surface plasmonic resonance wave~\cite{willets2007localized}, but the physical origin is different. 
The oscillation is due to the reflected polarization enhanced by the dielectric confinement, characterized by parameters~$\gamma_{\mrm{u}}$,~$\gamma_{\mrm{d}}$, and~$H$.
...
\end{equation}
where~$k_0 = \frac{\ln{(\gamma_{\mrm{u}} \gamma_{\mrm{d}})}}{2 H}$ is the first-order pole in the Sommerfeld integral representation of the Green's function.
Numerical validation shows that Eq.~\eqref{eq:wavelength} is highly robust under different choices of~$\V{r}$,~$\V{r}_0$,~$\gamma$, or~$H$, as shown in Fig.~\ref{fig:k_wavelength}.
...
\begin{figure}[htbp]
    ...
    \caption{
        Numerical validations for the relationship between~$k_0$ and~$\lambda$ under various system parameter settings of~$\gamma$ and~$H$. For each case, $\lambda$ is approximated by averaging distances between nearby zeros of~$E_x$, and with different (randomly generated) locations in~$z$. 
    }
    \label{fig:k_wavelength}
\end{figure}
