In this chapter, we focus on the quasi-2D Coulomb systems in uniform dielectric media, and introduce a fast algorithm named the sum-of-exponentials Ewald2D (SOEwald2D) method, together with its random batch accelerated variant (RBSE2D), which reaches an optimal time complexity of $O(N)$ for the force calculation and can easily handle the systems with large aspect ratios.
...
\begin{equation}\label{eq:inverse_laplace}
	e^{-x^2} = \frac{1}{2 \pi \m{i}} \int_{\Gamma} e^{z} \sqrt{\frac{\pi}{z}} e^{-2\sqrt{z}\abs{x}} dz\;,
\end{equation}
...
Specifically, the VPMR method integrates the flexibility of Vall\'ee-Poussin sums into the MR technique, demonstrating the highest convergence rate of $O(9^{-M})$ in constructing SOE approximation for Gaussians. 
This method is also bandwidth-controllable and uniformly convergent~\cite{AAMM-13-1126}. 
Due to these advantages, we will utilize the VPMR as the SOE construction tool in all the numerical experiments throughout this thesis.
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{split}
	\end{equation}
	where from the second to the third line, one uses the boundedness of SOE approximation error, i.e., Eq.~\eqref{eq::SOE1}.
	...
	\end{split}
	\end{equation}
	Again, the boundedness of the SOE approximation error is used in the proof, i.e., from the second to the third line.
\end{proof}
...
Achieving these properties is crucial for the subsequent algorithm design.

\begin{rmk}
	The choice of SOE approximation is not unique. Instead of approximating the Gaussian factor in the integrand with an SOE, a more straightforward approach might be approximating the complementary error function in~$\xi^{\pm}$ using an SOE. 
...
\begin{equation}\label{eq::SOEphi}
	\begin{split}
		...
		&=\frac{2\alpha e^{-h^2/(4\alpha^2)}}{\sqrt{\pi}h}e^{\m{i}\V{h} \cdot \V{\rho}_{ij}} \sum_{l = 1}^{M}  \frac{w_l}{\alpha^2 s_l^2 - h^2}\left( 2 \alpha s_l e^{-h z_{ij}} - 2he^{-\alpha s_l z_{ij}}\right) \;.
	\end{split}
\end{equation}
...
\begin{equation}\label{eq::Ul0SOE}
	U_{\ell,\text{SOE}}^{\bm{0}} \approx \frac{1}{2} \sum_{i = 1}^N q_{i} \phi^{\bm{0}}_\ell(\V{r}_{i}) = - \frac{2\pi}{L_xL_y}\sum_{1\leq j < i \leq N}q_{i}q_{j}\varphi^{\bm{0}}_{\text{SOE}}(\bm{r}_{i},\bm{r}_{j})-\frac{\sqrt{\pi} Q}{\alpha L_xL_y},
\end{equation}
where
...
\end{equation}
Clearly, $A_1(\beta)=0,$ $A_2(\beta)=q_1e^{-\m{i} \bm{h}\cdot\bm{\rho}_1}$, and for $i\geq 3$, a recursive algorithm can be constructed to achieve $O(N)$ complexity in computing all the coefficients:
...
Besides the iterative method discussed above, it is remarked that other fast algorithms based on SOE for 1D kernel summations have been developed~\cite{jiang2021approximating,GIMBUTAS2020815}, which can also be used under the framework described in this thesis for the summation in the non-periodic direction.
...
In MD simulations, the force exerted on the $i$-th particle, $\bm{F}_{i}$, plays a significant role in the numerical integration of Newton's equations.
One can similarly develop fast recursive algorithms to evaluate the SOE-reformulated forces,
the detailed expressions for $\bm{F}_{i}$ are summarized in Appendix~\ref{app::force}.
We finally summarize the SOEwald2D method in Algorithm~\ref{alg:SOEwald2D}. Its error and complexity analysis will be discussed in the next sections.
...
\begin{algorithm}[ht] 
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{Precomputation stage}: Determine Ewald splitting parameters $\alpha$ and $s$ according to Eq.~\eqref{eq:truncation_error}. Generate real and Fourier space cutoffs by $r_c=s/\alpha$ and $k_c=2s\alpha$, respectively. Construct the SOE approximations of $\xi^{\pm}(h,z)$ and $\erf(\alpha z)$ following Section~\ref{subsec::SOEapp}.
...
\begin{equation}
	\mathscr{E}_{U} := \mathscr{E}_{U_{s}}(r_c, \alpha) + \mathscr{E}_{U_{\ell}}(k_c, \alpha) + \sum_{\bm{h}\neq\bm{0}}\mathscr{E}_{U_{\ell},\text{SOE}}^{ \bm{h}} + \mathscr{E}_{U_{\ell},\text{SOE}}^{\bm{0}}\;,
\end{equation}
...
The remaining two terms are the error due to the SOE approximation for the Fourier space components, where
\begin{equation}
...
\end{equation}
Theorem~\ref{thm:SOE_error} provides upper bound error estimates, when the Debye--H\"uckel (DH) theory is assumed (see~\cite{levin2002electrostatic}, and also Appendix~\ref{app::Debye}) to approximate the charge distribution at equilibrium.
...
Based on Proposition~\ref{prop::2.12} and Theorem~\ref{thm:SOE_error}, we conclude that the overall absolute error in $U$ scales as $\mathscr{E}_{U}\sim O(\varepsilon N)$. 
...
For sorting (Step 4 in Algorithm~\ref{alg:SOEwald2D}), taking advantage of the quasi-2D confinement, various sorting algorithms are suitable, for example, the bucket sorting algorithm~\cite{cormen2022introduction} results in an $O(N)$ complexity.
...
Meanwhile, the total cost $ C_{\ell}$ in computing $U_{\ell,\text{SOE}}^{\bm{h}}$ for all $0<|\bm{h}|\leq k_c$ and $U_{\ell,\text{SOE}}^{\bm{0}}$ is given by
...
\begin{rmk}\label{rmk::extreme}
	...
	\begin{equation}
		C_{s}\sim 2\pi r_c^2 \frac{N}{L_x L_y} N=\frac{2\pi s^2N^2}{\alpha^2L_x L_y}.
	\end{equation}
...
\begin{equation}
	\mu:=\sum_{\bm{h}\in\mathcal{K}^2}\frac{f(\bm{h})}{g(\bm{h})}g(\bm{h})=\mathbb{E}_{\bm h\sim g(\bm{h})}\left[\frac{f(\bm{h})}{g(\bm{h})}\right],
\end{equation}
...
\begin{equation}
	\widetilde{\varphi} ({\bm{h}}) := e^{-\frac{h^2}{4 \alpha^2}} \left[ \frac{2\alpha\sqrt{\pi}}{L_xL_y}\sum_{l = 1}^{M}  \frac{w_l \sum_{1\leq j < i \leq N}q_{i} q_{j} e^{\m{i}\V{h} \cdot \V{\rho}_{ij}} \left(2\alpha s_l e^{-h z_{ij}}-2h e^{-\alpha s_l z_{ij}}\right) }{h(\alpha^2 s_l^2 - h^2)} \right] \;.
\end{equation}
...
\begin{equation}\label{eq::Happ}
	S=\frac{\alpha^2 L_xL_y}{\pi}\sum_{m_x,m_y\in\mathbb{Z}}e^{-\alpha^2(m_x^2L_x^2+m_y^2L_y^2)}-1,
\end{equation}
where $m_{\xi}$ denotes the dual summation index in the $\xi$ direction, $\xi\in\{x,y\}$. 
...
Then using the Metropolis algorithm~\cite{metropolis1953equation, hastings1970monte} (see Appendix~\ref{app::Metropolis}), a random mini-batch of frequencies $\{\bm{h}_{\eta}\}_{\eta=1}^P$ is sampled, and the Fourier component of energy can be approximated as:
...
\begin{equation}
	\begin{split}
		\widetilde{\varphi}^{\text{RB}}({\bm{h}}) e^{ - \frac{h^2}{4\alpha^2}} = \widetilde{\varphi}({\bm{h}}) \;,
	\end{split}
\end{equation}
...
By using the random batch approximation Eq.~\eqref{eq::RBapp}, the number of frequencies to be considered is then reduced to $O(P)$ per step, and  the complexity for the Fourier part is $O(PN)$, even for the challenging cases where the system is ultra-thin, i.e., $H \ll \min\{L_x,L_y\}$.
...
\begin{algorithm}[ht] 
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{Precomputation}: Determine Ewald splitting parameters $\alpha$ and $s$ according to Eqs.~\eqref{eq::59} and \eqref{eq:truncation_error}, respectively. Generate real space cutoff by $r_c=s/\alpha$. Construct the SOE approximation of $\xi^{\pm}(h,z)$ and $\erf(\alpha z)$ following Section~\ref{subsec::SOEapp}.
...
\begin{equation}\label{eq::xichi}
	\Xi := \sum_{\bm{h}\neq\bm{0}}U_{\ell}^{\bm{h}}-U_{\ell,*}^{\bm{h}\neq\bm{0}},\quad\text{and}\quad\V{\chi}_{i} :=\sum_{\bm{h}\neq\bm{0}}\left(\bm{F}_{\ell}^{\bm{h},i}-\bm{F}_{\ell,*}^{\bm{h},i}\right)\;.
\end{equation}
Proposition~\ref{prop:unbiased} is obtained directly by the definition of the importance sampling:
\begin{prop}\label{prop:unbiased}
	...
	\end{equation}
\end{prop}
% The unbiased property in Proposition~\ref{prop:unbiased} implies consistency of the random batch importance sampling. 
...
Furthermore, under the Debye--H\"uckel approximation, one has the following~Lemma~\ref{lem::upper_bound_phiRB} for the upper bounds of random batch approximations.
...
\end{lem}
\begin{proof}
	By the definition of $\widetilde{\varphi}^{\text{RB}}(\bm{h})$, one has
	\begin{equation}
...
Finally, by Lemma~\ref{lem::upper_bound_phiRB}, one has the following Theorem~\ref{thm:unbiased} for the boundedness and convergence in the fluctuations originating from the random batch approximation.
\begin{thm}\label{thm:unbiased}
...
\end{thm}
\begin{proof}
	By Proposition~\ref{prop:unbiased} and the definition of the normalization factor $S$, one has
	\begin{equation}
		\begin{split}
			\mathbb{E}\Xi^2
	...
	\begin{equation}
		\begin{split}
			\mathbb{E}\Xi^2&\leq \frac{2\lambda_D^4Q^2S}{\pi PL_xL_y}\int_{\frac{2\pi}{L}}^{\infty}\frac{e^{-h^2/(4\alpha^2)}}{h^2}\left(\sqrt{\pi}+\frac{\alpha \varepsilon}{h}\right)^2 2\pi h\,dh\\
	...
	\begin{equation}\label{eq::focva}
		\begin{split}
			...
			&\leq \frac{\lambda_D^4 S q_i^4}{2PL_xL_y} \int_{\frac{2\pi}{L}}^{\infty}e^{-h^2/(4\alpha^2)}\left[3+\frac{\alpha}{\sqrt{\pi}}\left(1+\frac{2\sqrt{2}\varepsilon}{h}\right)\right]^2 2\pi h\,dh\\
			&\leq \frac{S}{P}\frac{4\sqrt{\pi}\alpha^3(3\sqrt{\pi}+\alpha)\lambda_D^4 q_{i}^4}{L_xL_y}.
		\end{split}
	\end{equation}
	
	Finally, by definition Eq.~\eqref{eq::hk}, $S$ has the following estimate:
	\begin{equation}\label{eq::happrx}
		\begin{split}
			S &= \sum_{\bm{h}\neq\bm{0}}e^{-h^2/(4\alpha^2)}\leq \frac{L_xL_y}{(2\pi)^2}\int_{\frac{2\pi}{L}}^{\infty} e^{-h^2/(4\alpha^2)}2\pi h\,dh\leq \frac{\alpha^2L_xL_y}{\pi}.
		\end{split}
	\end{equation}
...
Theorem~\ref{thm:unbiased} has demonstrated that the variance of force scales as $O(1/P)$, unaffected by the growth of the system size $N$, provided the same particle density $\rho_s$ or the same Debye length $\lambda_D$. 
...
For ease of discussion, consider the widely used NVT ensemble, and a thermostat is introduced to control the system's temperature, which ensures sampling from the correct distribution. 
...
\begin{equation}\label{eq::langevin}
	\begin{split}
		...
		& d \boldsymbol{p}_i = \left[\boldsymbol{F}_i - \gamma \frac{\boldsymbol{p}_i}{m_i}\right] d t + \sqrt{2 \gamma k_{\mathrm{B}} T} \, d \boldsymbol{W}_i,
	\end{split}
\end{equation}
where $\boldsymbol{W}_i$ are i.i.d. Wiener processes, $\gamma$ is the friction coefficient of the thermostat, $k_{\mathrm{B}}$ is the Boltzmann constant, and $T$ is the temperature.
Let $(\bm{r}_i^*, \bm{p}_i^*)$ be the phase space trajectory for Eq.~\eqref{eq::langevin}, where the exact force $\bm{F}_i$ is replaced by the random batch approximated stochastic force $\bm{F}_i^*=\bm{F}_i-\bm{\chi}_i$. We further suppose that masses~$m_{i}$ for all $i$ are uniformly bounded.
...
\begin{thm} (Strong Convergence)
	...
	\end{equation}
	where~$\Lambda(N)=\|\mathbb{E} \abs{\V{\chi}_{i}}^2\|_{\infty}$ is the upper bound for the variance in the random batch approximated force. In the Debye--H\"uckel regime, $\Lambda(N)$ is independent of $N$ (see Theorem~\ref{thm:unbiased}).
	\label{thm:rbm_consist}
\end{thm}

The proof of Theorem~\ref{thm:rbm_consist} is based on previous work~\cite{jin2021convergence,Ye2023IMA} for the original random batch method~\cite{jin2020random}. However, it should be noted that this theorem may fail to apply to the RBSE2D method due to the singularity of Coulomb kernel at the origin, which violates the required Lipschitz continuity and boundedness conditions. 
Additionally, one may be concerned that the errors introduced by the SOE approximation and Ewald decomposition might disrupt the convergence of the method. 
...
Finally, numerical results presented in Section~\ref{sec:md} also validate the effectiveness of the random batch method in capturing finite-time structures and dynamic properties, which aligns with the conclusions of Theorem~\ref{thm:rbm_consist}.
...
The rigorous proof for the convergence of random batch approximated dynamics with the NH thermostat remains open, whereas we expect Theorem~\ref{thm:rbm_consist} to still hold. 
...
Finally, we discuss the parameter selection for the RBSE2D method. The accuracy is influenced by three key parameters: the parameter $s$, which controls the truncation error of the Ewald summation; the number of exponentials $M$ in the SOE, which governs the SOE approximation error; and the batch size $P$, which affects the variance of the random batch approximation. For a prescribed tolerance $\varepsilon$, $s$ and $M$ can be determined using Eq.~\eqref{eq:truncation_error} and the convergence rate of the SOE method~\cite{gao2021kernelindependent}, respectively. One can pick the proper $s$ and $M$ to ensure that errors from these two components are both at the $O(\varepsilon)$ level. 
The determination of the optimal batch size $P$ is system-dependent and relies on numerical testing. Notably, it is empirically observed that a small $P$ is sufficient, thanks to the importance sampling strategy for variance reduction. Numerical results in Section~\ref{subsec::RBSE2D} show that choosing $P=120$ is adequate for coarse-grained electrolytes. 
...
The results are presented in Fig.~\ref{fig:error_fixn}(a) for different number of exponentials in the SOE. 
Specifically, $M=4$, $8$ and $16$ correspond to SOE approximation errors $\varepsilon =10^{-4}$, $10^{-8}$, and $10^{-14}$, respectively. 
The convergence behavior depicted in Fig.~\ref{fig:error_fixn}(a) is consistent with our theoretical findings, demonstrating both a decaying rate of $O(e^{-s^2}/s^2)$ and a saturated precision of $O(\varepsilon)$ for the SOEwald2D method. 
...
The results presented in Fig.~\ref{fig:error_fixn}(b) reveal that the error is nearly unaffected by the size of the system, which aligns with the analysis presented in Section~\ref{sec::ewald2d}.
...
The Ewald2D truncation parameter~$s= 3, 4, 5$ are chosen for~$M = 4, 8, 16$, respectively, to obtain optimal accuracy as is guided by Fig.~\ref{fig:error_fixn} (a).
...
The results presented in Fig.~\ref{fig:error_Lz} clearly illustrate that the error of the Ewald2D method increases rapidly with $H$.
...
For many existing methods, the accurate evaluation of the forces exerted on particles can be strongly influenced by the particle's location in~$z$. 
...
Fig.~\ref{fig:error_ef}(a) clearly demonstrates that the pointwise error in force is independent of the particle's position in $z$. 
...
Fig.~\ref{fig:error_ef}(b) indicates that for such non-neutral slabs case, the pointwise error in forces calculated by the SOEwald2D method remains independent of $z$.

\begin{figure}[ht]
	\centering
	\includegraphics[width=\textwidth]{figs/fig_error_ef_unit.pdf} 
	\caption{
...
\begin{figure}[ht]
	...
	\begin{minipage}[c]{\textwidth}
		\centering
		\includegraphics[width=0.6\textwidth]{figs/hist_norm_unit.pdf}
	\end{minipage} \\
	\begin{minipage}[c]{\textwidth}
		\centering
...
Following the primitive model~\cite{frenkel2023understanding}, ions are represented as soft spheres with diameter $\sigma$ and mass $m$, interacting through the Coulomb potential and a purely repulsive shifted-truncated LJ potential. 
...
The simulation box has dimensions $L_x = L_y = 100 \sigma$ and $H = 30 \sigma$, where the ions are confined within the central region by purely repulsive LJ walls located at $z = 0$ and $z = 30 \sigma$ with $\epsilon_{\text{wall}} = \epsilon_{\text{LJ}}$ and $\sigma_{\text{wall}} = 0.5 \sigma$. 
The system contains $218$ cations and $218$ anions, and both walls are neutral. 
The simulation is performed with the time step~$\Delta t=0.001\tau$, where~$\tau = \sqrt{m \sigma^2 / \eps_{\text{LJ}}}$ denotes the LJ unit of time. 
The temperature is maintained by using a Nos\'e-Hoover thermostat~\cite{frenkel2023understanding} with a relaxation time of $0.1\tau$, fluctuating near the reduced external temperature $T=1$.
...
The ion concentration along the $z$-direction is measured, and presented in Fig.~\ref{fig:norm}. 
...
Furthermore, one evaluates the MSDs along both the periodic dimensions (Fig.~\ref{fig:norm}(b)) and the non-periodic dimension (Fig.~\ref{fig:norm}(c)), which describe the particles' anisotropic dynamic properties across a wide range of time scales. 
The RBSE2D methods for all $P$ yield almost identical MSD results as the SOEwald2D method. 
The confinement effect in $z$ leads to an $\mathrm{MSD}_z$ profile that clearly indicates a subdiffusion, while $\mathrm{MSD}_{xy}$ exhibits a normal diffusion process. 
Clearly, the RBSE2D method successfully captures this anisotropic collective phenomenon.


\begin{figure}[ht]
	\centering
	\includegraphics[width=0.49\textwidth]{figs/hist_Ez_unit.pdf} 
	\includegraphics[width=0.49\textwidth]{figs/hist_cation_unit.pdf}
	\caption{
...
The resulting equilibrium concentration of cations is shown in Fig.~\ref{fig:Ez_density}(a), indicating that results of RBSE2D method with different batch sizes are in good agreement with that of the reference SOEwald2D method. 
% The subplot of Fig.~\ref{fig:Ez_density}(a) also depicts the convergence as the batch size $P$ increases, which is consistent with our analysis.
...
In particular, the system consists of $436$ monovalent cations and is confined by slabs with surface charge densities $\sigma_{\mathrm{bot}}=0$ and $\sigma_{\mathrm{top}}=-0.0436$ to ensure overall charge neutrality. The concentration of free ions is depicted in Fig.~\ref{fig:Ez_density}(b), exhibiting excellent agreement with the results obtained using the SOEwald2D method. 
% The subplot in Fig.~\ref{fig:Ez_density}(b) also demonstrates the expected convergence of the RBSE2D method as the batch size increases. 
...
All calculations are performed on a Linux system equipped with an Intel Xeon Platinum 8358 CPU (2.6 GHz, 1 single core); and using a package we developed in the Julia language. 
To ensure a fair comparison, we maintain the same accuracy across all methods. We fix $s=4$ and set $M=8$ for the SOE approximation, resulting in errors at~$\sim10^{-8}$ for both the Ewald2D and SOEwald2D methods. %as shown in Fig.~\ref{fig:error_fixn}(a).
...
The CPU time comparison results are summarized in Fig.~\ref{fig:times_compare}. 
...
In Fig.~\ref{fig:times_compare}, it is demonstrated that when utilizing a single CPU core, the Ewald2D and SOEwald2D methods are limited to simulating system sizes of up to about $3 \times 10^4$ and $3 \times 10^5$ particles, respectively. 
In contrast, the RBSE2D method can handle systems containing about $5 \times 10^6$ particles. 
This is attributed to the much smaller real space cutoff $r_c$ that the RBSE2D algorithm can afford, which reduces the number of interacting neighbors that need to be stored. 
