In this chapter, we first present the mathematical formulation of the dielectric-confined quasi-2D Coulomb systems, along with its analytical solution based on image charges.
Then we revisit the Ewald summation method for doubly periodic lattice sums and its extension to dielectric-confined cases by being combined with the image charge method, as well as an reformulation of the ICM-Ewald2D summation.
% Finally, we introduce the random batch sampling method, which is a powerful method to accelerate the computation of the electrostatic energy.
...
We first introduce the case where the dielectric constants are equal throughout the system, i.e.,~$\eps_{\mrm{u}} = \eps_{\mrm{d}} = \eps_{\mrm{c}}$. For simplicity and without loss of generality, we set~$\eps_{\mrm{u}} = \eps_{\mrm{d}} = \eps_{\mrm{c}} = 1$.
...
For dielectric confined systems, i.e.~$\eps_{\mrm{u}}, \eps_{\mrm{d}} \neq \eps_{\mrm{c}}$, the Green's function $G(\V{r},~\V{r}^\prime)$, which describes the electrostatic response at any target location $\V r\in\mathbb{R}^3$ due to a point source charge located at $\V{r}^\prime\in\Omega$, satisfies Poisson's equation with dielectric interface conditions:
\begin{equation}
    \left\{
    \begin{array}{ll}
...
\section{Revisiting the Ewald Summation Formula}
