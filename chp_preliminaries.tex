...
\subsection{Ewald2D Summation and its Error Estimation} \label{sec::ewald2d}
...
\begin{itemize}
	...
	\item The function $\xi^{\pm}(k, z_{ij})$ is ill-conditioned: It grows exponentially as $kz_{ij}$ grows, leading to catastrophic error cancellation in actual computations with prescribed machine precision.
\end{itemize}
...
\subsection{The ICM-Ewald2D Summation and its Reformulation}\label{sec::icm_ewald2d}
...
In this section, we briefly introduce the idea of random mini-batch sampling and its application to fully-periodic Coulomb systems.
