...
Please find enclosed a revised version of my thesis. I would like to thank the committee members for their valuable comments and suggestions, which greatly help improve the quality of this thesis. I have carefully revised the thesis. A file identical to the revised version of the thesis, but with changes marked again in blue are attached. Below, itemized responses and corrections to all the committee members' comments are provided.


\begin{enumerate}

\item In the introduction, boundary element methods are suggested to be included to provide a more comprehensive introduction to the topic.

\ar{
    Thank you for the suggestion; it is true that boundary element methods are a powerful tool for solving the Poisson equation with complex boundary conditions. A brief introduction to boundary element methods has been added to the introduction (pages 3 and 4).
}
...
\ar{
    Thank you for pointing out the missing units. It is true that in Figs. 4.4, 4.5 and 4.6, the units are missing. Units have been added to the figures in the revised version.
}

\item Proposition 2.2.7 is not well written; the word ``well-defined" needs clarification.

\ar{
    The word ``well-defined'' here refers to the potential satisfying the infinite boundary conditions, which is a necessary condition for the potential to be mathematically well-defined. A more detailed explanation has been added to the proposition.
}

\item The author developed several useful packages, which are a good reference for junior students to get started. I hope that they can be briefly introduced in the thesis, maybe in the appendix.

\ar{
    A new chapter has been added as Appendix E to introduce the packages developed by the author. I introduced three main packages: \texttt{ExTinyMD.jl}, \texttt{EwaldSummations.jl} and \texttt{ChebParticleMesh.jl}, which are useful for the researchers who are interested in this thesis and the related topics.
}
